...
bool newJob(int pk, int iFrom, string strWorkers)
{
	...
	string strWorker;
	set<int> setWorkers;
	if((strWorkers.find("All") == string::npos) && (strWorkers.length() != 0))
	{
	...
	if(!sendMsg(WORKER, iFrom, strAction, pk, "", strWorkers, false))
		return false;
	...
	if((strWorkers.find("All") != string::npos) || (strWorkers.length() == 0))
	{
		string strContent;
		fDat >> strContent;
		if(!sendMsg(WORKER, iFrom, "data", pk, strContent,"All", false))
			return false;
	}
	fDat.close();
...
bool recJob(bool bSend)
{
	...
	while (!g_RecStack.is_empty())
	{
		...
		if(g_tasks.isFinished(pk,g_mWorkerDetails.size()))
		{
			string strBlocks = g_tasks.getBlock(pk);
			if(!sendMsg(GP, g_iWorker, "recBlock", pk, strBlocks, "All", false, g_iWorker))
				return false;
			if(!sendMsg(WORKER, g_iWorker, "recBlock", pk, strBlocks, "All", false, g_iWorker))
				return false;
		}
	}
...
//#endif
			while(iSolution < RETRY)
			{
				...
				if(std::find(vCancel.begin(), vCancel.end(), pk) != vCancel.end())
				{
					return true;
				}
			}
