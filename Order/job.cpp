...
bool recBlock(int iPK, int iFrom, string strMsg)
{
	...
	if(iMaxHeight > 0)
	{
		...
		printf("report to orderer: %s\n",strBlocks.c_str());
//		if(!sendMsg(EXCL, g_iWorker, "recBlock", iPK, strBlocks, "All", false, g_iWorker))
//			return false;
	}
	return true;
}

bool repBlk(int iPK, int iFrom, string strMsg)
{
	Blk *tBlk = new Blk();
	tBlk->strBlk = to_string(iPK) + " " + to_string(iFrom) + " " + strMsg;
	tBlk->preBlk = g_blk;
	g_blk = tBlk;

	sendMsg(GPs, g_iWorker, "disBlock", iPK, strMsg, "All", false, iFrom);
	return true;
}
