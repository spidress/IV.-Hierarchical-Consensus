...
int section = 20;
int interval = 60*1000; //milliseconds
...
Tx* getTx(Tx* genesis, int Worker)
{
	set<Tx*> suc = genesis->getsucWorkers();
	for(set<Tx*>::iterator it = suc.begin();
		it != suc.end();
		it++)
	{
		if(Worker == (*it)->getWorker())
			return *it;
		else
			return getTx(*it, Worker);
	}
	return NULL;
}
...
bool Tasks::recTx(int t_TP_public_key, int t_Worker, int t_Solution, int t_height, set<int> t_preIntWorkers)
{
	...
	if(t_height == 1)
	{
		if(TaskGenesises.find(t_TP_public_key) == TaskGenesises.end())
		{
			t = new Tx(t_TP_public_key, -1, 0, 0);
...
//		printf("reputation of %d = %lf * (%d - %d) ) / (double)%d * %lf / 2 = %lf\n", 
//			(*it)->getWorker(), 
//			per,
//			TipHeight, 
//			(*it)->getHeight(),
...
//	if(!loadWorkers(g_mWorkerDetails)) return false;
	for(map<int, vector<Tx*>>::iterator it = allWorkers.begin();
		it != allWorkers.end();
		it++)
	{
