...
bool loadAllWorkers(map<int, string> &g_mWorkerDetails)
{
	...
	{
		fWorkers.close();
		printf("loadAllWorkers err, cannot open Workers.txt\n");
		return false;
	}
...
bool loadAllGPs(map<int, string> &g_mGroups)
{
	...
	bool bWork = false;
	//load workers' details
	ifstream fGroups("Groups.txt");

	if(!fGroups)
	{
		fGroups.close();
		printf("cannot open Groups.txt\n");
		return false;
	}

	while(getline(fGroups, strWorkerDetail))
	{
		stringstream ss(strWorkerDetail);
		ss >> iWorkerTmp >> strIP >> iPort >> bWork;
		g_mGroups.insert(pair<int, string>(iWorkerTmp, strWorkerDetail));
	}
	fGroups.close();
	return true;
}
...
bool loadWorkers(map<int, string> &g_mWorkerDetails)
{
	...
	{
		fWorkers.close();
		printf("loadWorkers error, cannot open Workers.txt\n");
		return false;
	}
