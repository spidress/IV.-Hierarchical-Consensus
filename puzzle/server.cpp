...
//		}
		...
		buff[len] = '\0';
		printf("recv msg from client: %s at %.0lf\n", buff, loadTime());
		sprintf(szCmd, "echo %s %.0lf >> server.log", buff, loadTime());
		system(szCmd);
...
//			}
		...
		else if(strAction == "stop")
		{
			printf("stop to exit\n");
			//alert other nodes
			sendMsg(GPs, iFrom, strAction, 0, "", "", false);
#ifdef _WIN32
			Sleep(SLEEPTIME);
#else
		...
		//ask query
		else if(strAction == "report")
		{
			thread tReport(repBlk, pk, iFrom, strMsg);
			tReport.detach();
		}
